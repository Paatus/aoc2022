...
const solve1 = (input: string): number => {
    // const res = await walk(toMap(input), [`0,0`]);
...
const toLetter = (s: number) => {
    return String.fromCharCode(s + 97);
}

const drawMap = (map: Map<StrPos, Tile>) => {
    let acc = "";
    for (let y = 0; y < 40; y++) {
        for(let x = 0; x < 167; x++) {
            const tile = map.get(`${y},${x}`);
            // console.log(`${y},${x}`, tile);
            if (tile?.visited) { acc = `${acc}*` }
            else if (tile?.isEnd) { acc = `${acc}E` }
            else { acc = `${acc}${toLetter(tile!.height)}` }
        }
        acc = `${acc}\n`
    }
    console.log(acc);
